...
/***************************************************************************
 DiversityCalc
                                 A QGIS plugin
 Calculates biodiversity indexes (cabirols, guineus i perdius)
...
class DiversityCalc:
    ...
    def __init__(self, iface):
        ...
        self.actions = []
        self.menu = self.tr(u'&Natural Parks Diversity Calculator')
    ...
    def unload(self):
        ...
        for action in self.actions:
            self.iface.removePluginVectorMenu(
                self.tr(u'&Natural Parks Diversity Calculator'),
                action)
    ...
    def run(self):
        ...
        if result:
            ...
            # Get reqquiered input parameters from the dialog
            ...
            for category, summary in dctMain.items():
                ...
                twiCat = QTreeWidgetItem(dlgResults.trwResult, [category, str(total), str(dc_richness(summary)), "{:3.3f}".format(dc_evenness(summary)), "{:3.3f}".format(dc_shannons(summary)), "{:3.3f}".format(dc_simpsons(summary))])
                for species, obs in summary.items():
                    twiCat.addChild(QTreeWidgetItem(twiCat, [species, str(obs)]))
                dlgResults.trwResult.addTopLevelItem(twiCat)
