...
class PipelinePlanner:
    ...
    def __init__(self, iface):
        ...
        self.rbPipeline.setColor(Qt.red) # change the color
        self.rbPipeline.setWidth(4)      # change width
        ...
        self.actions = []
        self.menu = self.tr(u'&PipelinePlanner')

        # Check if plugin was started the first time in current QGIS session
        # Must be set in initGui() to survive plugin reloads
        self.first_start = None
        self.dlg = PipelinePlannerDialog()
        #width columns dialog
        self.dlg.tblImpacts.setColumnWidth(1, 50)
        self.dlg.tblImpacts.setColumnWidth(1, 155)
    ...
    def evaluatePipeline(self, point, button):
        if button == Qt.LeftButton:
            ...
            # QMessageBox.information(None, "Pipeline", pipeline.asWkt()) # wkt well known text
            ...
            for raptor in raptors:
                ...
                valDistance = pipeline.distance(raptor.geometry().centroid()) # distance center of buffer(centroid)
            ...
            for eagle in eagles:
                valConstraint = "BAEA Nest" # nomes tenim una especie deagles
                valID = eagle.attribute("nest_id")
                valStatus = eagle.attribute("status")
                valDistance = pipeline.distance(eagle.geometry().centroid())  # distance center of buffer(centroid)
            ...
            for buowl in buowls:
                valConstraint = "BUOWL Habitat"  # nomes tenim una especie deagles
                valID = buowl.attribute("habitat_id")
                valStatus = buowl.attribute("recentstat")
                valDistance = pipeline.distance(buowl.geometry().buffer(-0.001, 5))  # distance center of buffer(centroid)
